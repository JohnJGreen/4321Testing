...
public class RBTree {
    ...
    //CHANGED : Set to public to allow testing
    public int fixUpTree(RBNode z) {
        ...
        while (!z.Parent.Black) {
            if (z.Parent == z.Parent.Parent.Left) {
                ...
                //CHANGED : DEFECT - y may be null if grandparent is infinity node
                if (y == null) {
                    z.Parent.Black = true;
                    counter++;
                }else if (!y.Black) {
                ...
                if (!y.Black) {
                    ...
                    z.Parent.Parent.Black = false;
                    //CHANGED : DEFECT - Parent should be flipped to black
                    z.Parent.Black = true;
    ...
    public int delete(int k) {
        ...
        //CHANGED : DEFECT - z may be null if delete done on empty tree, causing null pointer exception
        if (z == null || z.Key != k ) {
        ...
        if (isNullNode(z.Left)) {
            ...
            y.Left = z.Left;
            y.Black = z.Black;
            //CHANGED - DEFECT: Left side of tree parent needs to be changed to y
            y.Left.Parent = y;
        }
        ...
        while (x != this.Root.Left && x.Black) {
            if (x == x.Parent.Left) {
                ...
                if (w.Left.Black && w.Right.Black) {
                    ...
                    if (w.Right.Black) {
                        //case 3 : Black, Left child, sibling has red child
                        w.Left.Black = true;
                        w.Black = false;
                        rightRotate(w);
                        w = x.Parent.Right;
     ...
     */
    public String min() {
        //CHANGED : DEFECT - if minimumNode returns null calling .Value causes null pointer exception
        //added check for empty tree
        if (this.empty()){
            return null;
        }
    ...
    private static RBNode minimumNode(RBNode node) {
        if (isNullNode(node)) {
            //UNREACHABLE CODE WITH min() change
            return null;
        }
    ...
    public static int sizeCalc(RBNode node) {
        ...
        if (!isNullNode(node.Left) && !isNullNode(node.Right)) {
            //CHANGED : DEFECT - Should return 1 for this node, not 2
...
//CHANGES: Made fields public to open values to JUnit tests
    public class RBNode {
        public String Value;
        public int Key;
        ...
        public String toString(){
            return this.Value + " " + this.Key + " " + this.Black;
        }
    }
}